...
async function onPanicFired() {
  if (selectedCode) {
    // Kod seçili → konum al, yakın yer kontrol et, SMS
    await sendEmergencySms(selectedCode);
  } else {
    // Kod seçili değil → sesli kod tanıma
...
async function sendEmergencySms(code) {
  ...
  // Yakında yer var mı kontrol et (15m)
  statusEl.textContent = '🗺️ YAKINDA YER ARANIYOR...';
  let nearbyPlaces = [];
  if (lat && lng) {
    nearbyPlaces = await getNearbyPlaces(lat, lng, 15);
  }

  if (nearbyPlaces.length > 0) {
    // Yer listesi göster — seçim yapılınca SMS gönderilecek
    showPlacePicker(code, locStr, mapsLink, coords, lat, lng, nearbyPlaces);
    statusEl.textContent = '';
    statusEl.className   = 'panic-status';
  } else {
    // Yakında yer yok — direkt SMS gönder
    finalizeSms(code, locStr, mapsLink, coords, lat, lng, null);
  }
}

// SMS'i tamamla ve gönder (yer seçimiyle veya seçimsiz)
...
function showSmsNotification(smsBody) {
  if (!('Notification' in window)) return;
  if (Notification.permission === 'granted') {
    const n = new Notification('Acil Durum İletisi - Tıklayın', {
      body: smsBody.substring(0, 100) + (smsBody.length > 100 ? '...' : ''),
      icon: '',
      tag:  'ocst-sms-command',
...
// ══════════════════════════════════════════════════════
// SMS KOMUTU OVERLAY (PC'den gelen)
// ══════════════════════════════════════════════════════
function showSmsCommandOverlay(smsBody) {
  // Varsa eskiyi kaldır
  const existing = document.getElementById('sms-cmd-overlay');
  if (existing) existing.remove();

  const overlay = document.createElement('div');
  overlay.id = 'sms-cmd-overlay';
  overlay.style.cssText = [
    'position:fixed;inset:0;z-index:999;',
    'background:rgba(4,10,20,.97);',
    'display:flex;flex-direction:column;align-items:center;justify-content:center;',
    'font-family:\'Share Tech Mono\',monospace;padding:24px;gap:20px;text-align:center;',
    'backdrop-filter:blur(6px);'
  ].join('');

  overlay.innerHTML = `
    <div style="font-size:11px;color:#ff4444;letter-spacing:4px;animation:blink-r 0.8s infinite;">🚨 ACİL KOMUT GELDİ</div>
    <div style="font-size:13px;color:#ffaa00;letter-spacing:2px;">PC'DEN SMS KOMUTU</div>
    <div style="background:rgba(10,20,40,.9);border:1px solid #1a3a5a;padding:16px;max-width:360px;width:100%;
                font-size:12px;color:#c8d8e8;line-height:1.7;letter-spacing:.3px;text-align:left;white-space:pre-wrap;word-break:break-word;">
      ${escH(smsBody)}
    </div>
    <button id="sms-cmd-send-btn" onclick="executeSmsCommand()"
      style="width:100%;max-width:360px;background:rgba(180,30,30,.3);border:2px solid #ff4444;
             color:#ff4444;font-family:'Share Tech Mono',monospace;font-size:16px;
             padding:18px;cursor:pointer;letter-spacing:3px;
             box-shadow:0 0 30px rgba(255,68,68,.4);">
      📱 SMS UYGULAMASINI AÇ
    </button>
    <button onclick="document.getElementById('sms-cmd-overlay').remove()"
      style="background:transparent;border:1px solid #3a5a7a;color:#6a8aaa;
             font-family:'Share Tech Mono',monospace;font-size:11px;padding:8px 24px;cursor:pointer;letter-spacing:1px;">
      KAPAT
    </button>
  `;

  // SMS metnini sakla
  overlay.dataset.smsBody = smsBody;
  document.body.appendChild(overlay);

  // Bildirimi de gönder (sekme arka plandaysa)
  showSmsNotification(smsBody);
}

function executeSmsCommand() {
  const overlay = document.getElementById('sms-cmd-overlay');
  if (!overlay) return;
  const smsBody = overlay.dataset.smsBody || '';
  const smsUri  = `sms:113?body=${encodeURIComponent(smsBody)}`;
  window.location.href = smsUri;
}
...
function connectWS() {
  ...
  try {
    ...
    ws.onopen = () => {
      setWSDot(true);
      // Sunucuya bu bağlantının hangi cihaza ait olduğunu bildir
      ws.send(JSON.stringify({ type: 'mobile_identify', deviceId }));
    };
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (msg.type === 'sms_command' && msg.smsBody) {
          showSmsCommandOverlay(msg.smsBody);
        }
      } catch {}
    };
...
// ══════════════════════════════════════════════════════
// YAKINDA YER SORGULAMA (Overpass API — OpenStreetMap)
// ══════════════════════════════════════════════════════

async function getNearbyPlaces(lat, lng, radiusM) {
  // Overpass: 15m çevresinde isimli bina, dükkan, apartman ara
  const query = `
    [out:json][timeout:6];
    (
      node(around:${radiusM},${lat},${lng})[name];
      way(around:${radiusM},${lat},${lng})[name][building];
  ...
  try {
    ...
    for (const el of (data.elements || [])) {
      ...
      const tags = el.tags || {};
      let type = '';
      if (tags.building === 'apartments' || tags.building === 'residential') type = 'Apartman';
      else if (tags.building)  type = 'Bina';
      else if (tags.shop)      type = 'İşletme';
      else if (tags.amenity)   type = 'Tesis';
      else if (tags.office)    type = 'Ofis';
...
function showPlacePicker(code, locStr, mapsLink, coords, lat, lng, places) {
  _pickerCode   = code;
  _pickerLoc    = locStr;
  _pickerMaps   = mapsLink;
  _pickerCoords = coords;
  _pickerLat    = lat;
  _pickerLng    = lng;

  const overlay = document.getElementById('place-picker-overlay');
  const list    = document.getElementById('place-picker-list');

  // Seçenekleri oluştur
  list.innerHTML = places.map((p, i) => `
    <button class="pp-item" onclick="selectPlace(${i})">
      <span class="pp-item-type">${p.type || 'Yer'}</span>
      <span class="pp-item-name">${escH(p.name)}</span>
    </button>
  `).join('') + `
    <button class="pp-item pp-item-none" onclick="selectPlace(-1)">
      <span class="pp-item-name">Hiçbiri</span>
    </button>
  `;

  // Yerleri data olarak sakla
  overlay.dataset.places = JSON.stringify(places);
...
function selectPlace(idx) {
  ...
  const places    = JSON.parse(overlay.dataset.places || '[]');
  const placeName = idx >= 0 ? places[idx]?.name : null;
